...
func GetToken(c echo.Context) error {
	...
	if r.ClientID == "" || r.ClientSecret == "" {
		r.ClientID = conf.Conf.Onedrive.ClientID
		r.ClientSecret = conf.Conf.Onedrive.ClientSecret
	}
	...
	zone, ok := zones[r.Region]
	if !ok {
		zone = zones["global"]
	}

	req, _ := http.NewRequestWithContext(c.Request().Context(), http.MethodPost, fmt.Sprintf("%s/common/oauth2/v2.0/token", zone.Oauth), strings.NewReader(params.Encode()))
